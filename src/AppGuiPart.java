...
public class AppGuiPart extends OracleConnector implements ActionListener , MouseListener {
    ...
    protected void signInGui(){
        ...
        try{
            ResultSet resultSet = giveQuery("select current_date from dual");
            resultSet.next();
...
//        signInL = new JLabel("SignUN");
    ...
    protected  void staffInformationView(){
        ...
        String supervisor = "";
        try {
            ResultSet resultSet = giveQuery("select initCap(first_name)|| ' '|| last_name as name from staff where id = "
...
//todo
            showTable("select * from given_tasks where staff_id = " + staff.getId(), taskedWork);
    ...
    protected  void showBookedEvents(){
        ...
        showTable("select initcap(first_name)|| ' '|| initcap(last_name) as Organizer , tellNo1, tellNo2 from\n" +
    ...
    protected   void bookEvent(){
        ...
        try {
            ResultSet eventResult = giveQuery("SELECT id || '.'|| initCap(name)|| ' max '|| capacity|| ' $'|| price," +
                    " price, id from venue");
                            ...
                            if (!guestList.contains(firstName + " " + lastName))
                                ...
                                System.out.println(newGuest.getId());
                                    if(newGuest.getId() != 0) {
                                        ...
                                        showTable("select Guest_Name from show_event_guests " +
                                                        "WHERE customer_id = "+ customer.getId() + " and event_id = " + givenEventId
                                                , showGuests);
                                        guestList.remove(firstName +" "+ lastName);
                                        messageLabel.setText((maxGuests - guestList.size()) +" Left");
                                        guestCount = maxGuests - guestList.size();
            ...
            if(staffSignInPasswd.getPassword().length > 3) {
                ...
                try {
                    if (!resultSet.next()) {
                        ...
                        phoneNo1 = resultSet.getString("tellNo1");
                        phoneNo2 = resultSet.getString("tellNo2");
                        appointedDate = String.valueOf(resultSet.getDate("apointed_date"));
        ...
        try {
            ...
            if(givenMoney >= eventCost) {
                ...
                try {
                    ...
                    ResultSet resultSet = giveQuery("select  count(*) as much from event where event_date = '" +
                            event.getEventDate() + "' and venue_id = " + event.getVenueId());
                    if (resultSet.next()){
                        dateCheck = resultSet.getInt("much");
                ...
                try {
                    ...
                    if (eventThere != 0) {
                        ...
                        try {
                            ...
                            try {
                                //todo show event guest
                                resultSet = giveQuery("select count(*) as countg from show_event_guests where event_id =" + givenEventId);
                                while(resultSet.next()) {
                                guestCount = resultSet.getInt("countg");
                                System.out.println(guestCount);
                                messageLabel.setText((maxGuests - guestCount) +" Left");
                                }
                                showTable("select Guest_Name from show_event_guests " +
                                                "WHERE customer_id = "+ customer.getId() + " and event_id = " + givenEventId
                                        , showGuests);
                                if (guestCount < maxGuests) {
                                    ...
                                    try {
                                        resultSet = giveQuery("select first_name || ' '|| last_name as \"Guest Name\"" +
                    ...
                    try {
                        if (guestCount < maxGuests) {
                            ...
                            if (guestList.contains(firstName + " " + lastName))
                                ...
                                try {
                                    if(newGuest.getId() == 0) {
                                        resultSet = giveQuery("select id from guest order by id");
                                        while(resultSet.next()) {
